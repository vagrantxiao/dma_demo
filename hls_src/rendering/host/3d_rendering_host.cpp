...
int main(int argc, char ** argv)
{
  ...
  // opencl version host code
    ...
    for ( int i = 0; i < NUM_3D_TRI; i++)
    {
      ...
      input[3*i](23,16)   = triangle_3ds[i].z0;
      input[3*i](31,24)   = triangle_3ds[i].x1;
      Input_1.write(input[3*i]);
      input[3*i+1](7,0)   = triangle_3ds[i].y1;
      input[3*i+1](15,8)  = triangle_3ds[i].z1;
      input[3*i+1](23,16) = triangle_3ds[i].x2;
      input[3*i+1](31,24) = triangle_3ds[i].y2;
      Input_1.write(input[3*i+1]);
      input[3*i+2](7,0)   = triangle_3ds[i].z2;
      input[3*i+2](31,8)  = 0;
      Input_1.write(input[3*i+2]);
    }
    */
    ...
    //Input_1.write(1);
    //data_send(Input_1, Output_1);


    //int out_temp =Output_1.read();

    //int out_temp = 0;
    //for(int i=0; i<1024; i++)
	//{
	//	out_temp = Output_1.read();
	//}
    printf("We finally get %d\n", MAX_PKT_LEN*NUMBER_OF_PACKETS*8/DATA_WIDTH);

    /*
    int tmp;
    for(int sb=0; sb<16; sb++){
    	tmp=(int) Output_1.read();
    }
